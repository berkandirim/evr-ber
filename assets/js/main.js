...
(() => {
  ...
  application.register("dateTime", class extends Stimulus.Controller {
    static targets = ['year', 'timeLeft'];
    initialize() {
      renderText(this.timeLeftTarget, `${ duration.months() } ay ${ duration.days() } gün ${ duration.hours() }:${ duration.minutes() }:${ duration.seconds() }`)
      setInterval(() => {
